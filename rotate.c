...
local int BMOBRotSize( BMOB *old, BMOB *new, LongFrac sin, LongFrac cos )
{
    SHORT  w, h, neww, newh;
    w = old->pict.box.w;
    h = old->pict.bm->Rows;
...
local BMRot( struct BitMap *obm, struct BitMap *nbm, LongFrac sin, LongFrac cos, SHORT width, SHORT xpcol )
{
...
local int sincos( SHORT dx, SHORT dy, LongFrac *sn, LongFrac *cs )
{
...
void XorLine( SHORT x0, SHORT y0, SHORT x1, SHORT y1 )
{
    SHORT svUsePen;
...
local void XorRotB1( SHORT x, SHORT y, SHORT w, SHORT h )
{
    LongFrac sn, cs;
...
local void XorRotBox( SHORT x, SHORT y )
{
...
local int invbrbox( SHORT x, SHORT y )
{
    Box bx;

    TempXOR();

    mFrame( MakeBox( &bx, x - curbr.xoffs, y - curbr.yoffs, curbr.pict.box.w, curbr.pict.box.h ) );
...
local void frobmv()
{
    invbrbox( lx, ly );  frobsh();
}

local void frobclr()
{
    invbrbox( mx, my );  obclr();
}

void IMRotBrush( BOOL restart )
{
    if ( !restart ) {
        ...
        if ( CopyBMOBPict( &origBr, &curbr ) == FAIL ) {
            RestoreBrush();
            RevertIMode(); return;
        }

        origBr.xoffs = curbr.xoffs;
        origBr.yoffs = curbr.yoffs;

        curbr.xoffs = curbr.pict.box.w;
        curbr.yoffs = curbr.pict.box.h;
    }
